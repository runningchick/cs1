...
-- A polymorphic set type. We interpret values of type (set a) as representing sets of values of type a.
...
set_product: (eq a) => (s1: set a) -> (s2: set b) -> set (pair a b)
...
--Please see implementation at bottom
-- Since I needed to use mkSet in my toStringSet function, it wouldn't compile correctly up here
...
-- Return union of s1 and s2
-- hint, write a function, combine, that combines two lists into one, avoiding duplicate elements
...
set_exists p (mkSet l) = (isEmpty (mkSet (list.filter p l)))
